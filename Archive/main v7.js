document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchNetworkGraph(skuId) {
        ...
        // Add the SKU context heading first
        const heading = document.createElement('h2');
        heading.classList.add('text-xl', 'font-bold', 'text-gray-800', 'mb-4');
        heading.textContent = `Network Graph for SKU: ${skuId}`;
        resultsContainer.appendChild(heading);

        // Create the container element dynamically and append it
        const container = document.createElement('div');
        ...
        .then(data => {
            // Get the container element after it's been added to the DOM
            // This time, we have a direct reference so this will work correctly
            ...
            data.forEach(path => {
                ...
                path.nodes.forEach(node => {
                    if (!uniqueNodeIds.has(node.id)) {
                        ...
                        // Define a pastel color palette for different node types
                        let color = { background: '#d1d5db', border: '#9ca3af' }; // Default gray
                        let shape = 'box'; // Default shape
                        let size = 20;

                        if (node.labels.includes('SKU')) {
                            color = { background: '#93c5fd', border: '#3b82f6' }; // Pastel Blue
                            shape = 'triangle';
                            size = 15; // Adjusted size for better visual balance
                        } else if (node.labels.includes('BOM')) {
                            color = { background: '#fdbf8b', border: '#f97316' }; // Pastel Orange
                            shape = 'circle';
                            size = 20; // Adjusted size for better visual balance
                        } else if (node.labels.includes('Res')) {
                            color = { background: '#a7f3d0', border: '#10b981' }; // Pastel Green
                            shape = 'box';
                            size = 20;
                        }
            ...
            const options = {
                ...
                edges: {
                    ...
                    font: {
                        ...
                        color: '#6b7280',
                        strokeWidth: 5,
                        strokeColor: '#ffffff', // Use white stroke for text background
                        align: 'middle' // Keep label centered on the edge
                    },
                    smooth: {
                        enabled: true,
                        type: 'straightCross' // Forces a straight line connection from end to end
                    }
                },
                // Use hierarchical layout with improved spacing
                physics: {
                    enabled: false
                },
                layout: {
                    hierarchical: {
                        ...
                        sortMethod: 'directed',
                        levelSeparation: 250, // Increased to make arrows longer
                        nodeSpacing: 150 // Increased to improve spacing
                    }
                },
                interaction: {
            ...
            if (networkContainer) {
                ...
                // Set the container height dynamically based on the network size
                network.on("stabilizationIterationsDone", function () {
                    const canvasHeight = network.getBoundingBox().height + 50; // Add some padding
